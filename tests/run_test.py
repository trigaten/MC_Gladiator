...
class TestEnvironments():
    ...
    def test_base_wrapper(self):
        ...
        rewards = wrapped_env.compute_rewards(a0_new_health=15, a1_new_health=20)
        assert rewards["agent_1"] > 0 and rewards["agent_0"] == 0

        # agent_0 should get reward, while agent_1 should get none
        rewards = wrapped_env.compute_rewards(a0_new_health=20, a1_new_health=15)
        assert rewards["agent_1"] == 0 and rewards["agent_0"] > 0
        ...
        # test normalization
        rewards = wrapped_env.compute_rewards(a0_new_health=11, a1_new_health=20)
        assert rewards["agent_1"] == 1

        # test noop stepping
        # noop = wrapped_env.get_noop()
        wrapped_env.step({"agent_0":2, "agent_1":0})
