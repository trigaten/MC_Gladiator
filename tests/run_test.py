...
class TestBaseWrapper():
    ...
    def test_reward_computation(self):
        ...
        rewards = wrapped_env.compute_rewards(a0_new_health=dh-5, a1_new_health=dh)
        assert rewards["agent_1"] > 0 and rewards["agent_0"] == 0

        # agent_0 should get reward, while agent_1 should get none
        rewards = wrapped_env.compute_rewards(a0_new_health=dh, a1_new_health=dh-5)
        assert rewards["agent_1"] == 0 and rewards["agent_0"] > 0

        # both agents should get negative reward
        rewards = wrapped_env.compute_rewards(a0_new_health=dh+1, a1_new_health=dh+1)
        assert rewards["agent_0"] < 0 and rewards["agent_1"] < 0

        # test normalization
        rewards = wrapped_env.compute_rewards(a0_new_health=dh-9, a1_new_health=dh)
    ...
    def test_attack_damage(self):
        for _ in range(500):
            num_actions = len(wrapped_env.actions)
            a1, a2 = random.randint(0, num_actions-1), random.randint(0, num_actions-1)
            _ = wrapped_env.step({"agent_0":a1, "agent_1":a2})
